...
export default function ProductDetailModal({ product, userPortfolio, onClose, onProceedToCheckout }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white border border-slate-200 rounded-2xl sm:rounded-3xl p-4 sm:p-7 shadow-2xl space-y-4 sm:space-y-6 my-4 sm:my-8 max-h-[92vh] overflow-y-auto">
        
        {/* Close Button */}
        <button 
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 rounded-full bg-slate-100 text-slate-500 hover:text-slate-900 hover:bg-slate-200 transition-colors z-10"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 items-start">
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            {/* Image Container */}
            <div className="relative aspect-[4/3] w-full rounded-xl sm:rounded-2xl overflow-hidden bg-slate-50 border border-slate-100 p-2 sm:p-4 flex items-center justify-center">
              <img 
                src={selectedColor?.image || product.image} 
                alt={product.title} 
                className="w-full h-full object-cover object-center rounded-lg sm:rounded-xl"
              />
              <span className="absolute top-2 left-2 sm:top-3 sm:left-3 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-purple-100 text-[#5b21b6] text-[10px] sm:text-xs font-bold flex items-center gap-1">
                <i className="bi bi-lightning-charge-fill text-[#6320ee]"></i>
                <span>{product.badge}</span>
              </span>
            </div>

            {/* Key Product Highlights */}
            <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-purple-50/50 border border-purple-100 space-y-2">
              <h5 className="font-bold text-purple-950 text-[11px] sm:text-xs uppercase tracking-wider flex items-center gap-1.5">
                <i className="bi bi-stars text-[#6320ee]"></i>
              ...
              <ul className="space-y-1.5 sm:space-y-2 text-xs text-slate-700">
                {product.highlights.map((highlight, idx) => (
                  <li key={idx} className="flex items-start gap-1.5 sm:gap-2">
                    <i className="bi bi-check-lg text-[#6320ee] font-bold text-sm shrink-0"></i>
                    <span>{highlight}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] sm:text-xs text-slate-600">
              <div className="p-2 sm:p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-1.5 sm:gap-2">
                <i className="bi bi-truck text-[#6320ee] text-sm shrink-0"></i>
                <span className="line-clamp-1">Delivery in 2-3 Days</span>
              </div>
              <div className="p-2 sm:p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-1.5 sm:gap-2">
                <i className="bi bi-shield-check text-emerald-600 text-sm shrink-0"></i>
                <span className="line-clamp-1">1 Year Warranty</span>
              </div>
            </div>
          ...
          <div className="space-y-4 sm:space-y-5">
            ...
            {/* Title & Brand */}
            <div>
              <div className="flex items-center gap-2 text-[11px] font-extrabold text-[#6320ee] uppercase tracking-wider mb-1">
            ...
            {/* Price Box */}
            <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between">
              <div>
                <span className="text-[11px] text-slate-500 font-medium">Total Price:</span>
                  ...
                  {product.variants.storage.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl border text-xs font-bold transition-all ${
                          isSelected 
                            ? 'bg-purple-100 border-[#6320ee] text-[#5b21b6] shadow-sm' 
                            : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-700'
                        }`}
            ...
            <button
              ...
              className="w-full py-3 sm:py-3.5 px-4 sm:px-5 rounded-xl sm:rounded-2xl bg-gradient-to-r from-[#6320ee] to-[#7c3aed] hover:from-[#531ad6] hover:to-[#6d28d9] text-white font-extrabold text-xs sm:text-sm uppercase tracking-wider flex items-center justify-center gap-2 shadow-xl shadow-purple-900/20 transition-all duration-200 transform hover:-translate-y-0.5"
            >
